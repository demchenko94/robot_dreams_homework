...
describe('createMultiplier', () => {
  ...
  it('should throw an error if input is not a number', () => {
    expect(() => createMultiplier('abc')).toThrow(
      'The provided value is not a number'
    );
    expect(() => createMultiplier(NaN)).toThrow(
      'The provided value is not a number'
    );
    expect(() => createMultiplier(undefined)).toThrow(
      'The provided value is not a number'
    );
    expect(() => createMultiplier(null)).toThrow(
      'The provided value is not a number'
    );
  });

  it('should treat numeric strings as invalid', () => {
    expect(() => createMultiplier('5')).toThrow(
      'The provided value is not a number'
    );
  });
});
